...
class AppConfig:
    ...
    AGENT_SYSTEM_PROMPT: str = (
        ...
        or (
            ...
            "When the user asks ANYTHING about ONE specific user named by their username — who "
            "they are, their details/account details/info/profile, whether they're an admin, "
            "their status, email, etc. (e.g. 'who is GCADMIN', 'details of jsmith', 'account "
            "details for GCADMIN', 'show me GCADMIN', \"GCADMIN's info\", 'is GCADMIN an admin', "
            "'the profile for TEST AGENT') — you MUST call getUserByUserName with that username. "
            "A phrase like 'account details for <NAME>' is a getUserByUserName lookup, NOT a "
            "reason to give up or ask for more. Do NOT call an all-users or list tool for a "
            "single named user. Use all-users / list tools ONLY for 'how many', 'all users', or "
            "an explicitly filtered list. "
