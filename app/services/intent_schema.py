...
_USER_PATS = [
    ...
    re.compile(r"what (?:applications?|apps?|access|roles?|fund ?groups?|offices?|organi[sz]ations?) (?:does|do|has|have) ([A-Za-z0-9._@-]+)", re.I),
    re.compile(r"what (?:does|do|has|have) ([A-Za-z0-9._@-]+)\b", re.I),
    re.compile(r"\baccess (?:for|of) ([A-Za-z0-9._@-]+)", re.I),
    re.compile(r"\b(?:applications?|apps?|access|roles?|fund ?groups?|offices?|organi[sz]ations?) (?:assigned|granted) to (?:the )?([A-Za-z0-9._@-]+)", re.I),
    re.compile(r"\bassigned to (?:the )?([A-Za-z0-9._@-]+)", re.I),
    re.compile(r"\b([A-Za-z0-9._@-]+)'s (?:access|applications?|roles?|fund ?groups?|offices?|organi[sz]ations?)\b", re.I),
...
def route(it: Intent) -> str:
    ...
    if e == "fund_group":
        ...
        return "fund_groups"
    if e == "organization":
        # "my offices" → the caller's; "<user>'s offices" → that named user's (guarded, app-aware,
        # never the master org list); a bare "organizations" → the catalogue.
        if s == "self":
            return "my_offices"
        if s == "user":
            return "user_offices"
